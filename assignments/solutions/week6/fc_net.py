...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss += 0.5 * self.reg*np.sum(np.square(self.params['W1'])) # adhere to the note
        loss += 0.5 * self.reg*np.sum(np.square(self.params['W2'])) #  regarding L2 reg
        
        dx2, dw2, db2 = affine_backward(dx_final, cache2)
        _  , dw1, db1 = affine_relu_backward(dx2, cache1) # dx1 can be ignored/discarded
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for layer in range(1, self.num_layers + 1):
            ...
            if self.normalization == "batchnorm" or self.normalization == "layernorm" and layer != self.num_layers:
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(1, self.num_layers + 1):
            if layer == self.num_layers:
                scores, cache = affine_forward(scores, self.params['W%d' % (layer)],
                ...
                if self.normalization == "batchnorm":
                    ...
                    scores, cache = affine_layernorm_forward(scores, self.params['W%d' % (layer)],
                                                                     self.params['b%d' % (layer)],
                                                                     self.params['gamma%d' % (layer)], 
                                                                     self.params['beta%d' % (layer)], 
                                                                     self.bn_params[layer])
        ...
        # add on regularization losses across layers
        for layer in range(1, self.num_layers + 1):
            loss += 0.5 * self.reg * np.sum(np.square(self.params["W%d" % layer]))

        # compute gradients from output layer to input layer
        dx = dout
        for layer in reversed(range(1, self.num_layers + 1)):
            if layer == self.num_layers:
                dx, grads["W%d" % layer], grads["b%d" % layer] = affine_backward(dx, cache_history.pop())
            else:
                ...
                if self.normalization == "batchnorm":
                    ...
                    grads["b%d" % layer], \
                    grads["gamma%d" % layer], \
                    grads["beta%d" % layer] = affine_layernorm_backward(dx, cache_history.pop())
...
def affine_layernorm_forward(x, w, b, gamma, beta, bn_param):
    """
    Convenience layer that performs 
        an affine transform, followed by 
        a layernorm transform, followed by
        a ReLU

    Inputs:
    - x: Input to the affine layer
    - w, b: Weights for the affine layer
    - gamma, beta, bn_param: params for layernorm
...
def affine_layernorm_backward(dout, cache):
    """
    Backward pass for the affine-layernorm convenience layer
    """
    fc_cache, bn_cache, relu_cache = cache
    da = relu_backward(dout, relu_cache)
